...
//? }
    ...
    private static void doVClipShoot(LocalPlayer p, ArrowDmgConfig cfg) {
        ...
        p.connection.send(new ServerboundMovePlayerPacket.PosRot(shootPos.x, shootPos.y, shootPos.z, yaw, pitch, false));
        ...
        p.setYRot(origYaw);
        p.setXRot(origPitch);
        p.connection.send(new ServerboundMovePlayerPacket.PosRot(orig.x, orig.y + 0.01, orig.z, origYaw, origPitch, false));
    ...
    private static void doDMG(ArrowDmgConfig cfg) {
        ...
        // ★ 参考 InvincibleMachineGun/ArrowDmg.java：先发疾跑包（START_SPRINTING），再做基于朝向的位置欺骗。
