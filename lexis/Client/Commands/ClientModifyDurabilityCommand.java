...
@EventBusSubscriber
public class ClientModifyDurabilityCommand {
   ...
   public static void onRegisterClientCommands(RegisterClientCommandsEvent event) {
      ...
      dispatcher.register((LiteralArgumentBuilder)Commands.m_82127_("lexis").then(Commands.m_82127_("client").then(((LiteralArgumentBuilder)Commands.m_82127_("ModifyDamagedurability").then(Commands.m_82127_("setdurability").then(Commands.m_82129_("value", IntegerArgumentType.integer(-2147483647, Integer.MAX_VALUE)).executes(ClientModifyDurabilityCommand::setDurability)))).then(Commands.m_82127_("setalldurability").then(Commands.m_82129_("value", IntegerArgumentType.integer(-2147483647, Integer.MAX_VALUE)).executes(ClientModifyDurabilityCommand::setAllDurability))))));
   }
   ...
   private static int setDurability(CommandContext ctx) {
      if (!checkCreativeMode(ctx)) {
         ...
         if (slot == -1) {
            ...
            if (stack.m_41619_()) {
               sendMessage(ctx, "物品是空");
               return 0;
            } else {
               ItemStack newStack = Utils.setItemDurability(stack.m_41777_(), value);
               ...
               if (networkSlot == -1) {
                  ...
                  sendMessage(ctx, "已修改物品 " + var10001 + " 耐久值 (新损伤值: " + newStack.m_41773_() + ")");
                  return 1;
               }
            }
         }
   ...
   private static int setAllDurability(CommandContext ctx) {
      if (!checkCreativeMode(ctx)) {
         return 0;
      } else {
         int value = IntegerArgumentType.getInteger(ctx, "value");
         ...
         for(int slot = 0; slot < 36; ++slot) {
            ...
            if (!newOffhand.m_41619_() && newOffhand.m_41776_() > 0) {
               ItemStack newStack = Utils.setItemDurability(newOffhand.m_41777_(), value);
               int networkSlot = toNetworkSlot(slot);
               if (networkSlot != -1) {
                  updateSlot(slot, networkSlot, newStack);
                  ++total;
         ...
         if (!offhand.m_41619_() && offhand.m_41776_() > 0) {
            newOffhand = Utils.setItemDurability(offhand.m_41777_(), value);
            updateSlot(40, 45, newOffhand);
            ++total;
         }

         sendMessage(ctx, "已修改 " + total + " 个物品的耐久值");
         return 1;
      }
   }
}
