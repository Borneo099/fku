...
public class TpGotoCommand {
   ...
   public static void onRegisterCommands(RegisterClientCommandsEvent event) {
      event.getDispatcher().register((LiteralArgumentBuilder)Commands.m_82127_("lexis").then(Commands.m_82127_("client").then(((LiteralArgumentBuilder)Commands.m_82127_("tpgoto").then(Commands.m_82129_("player", StringArgumentType.string()).suggests(playerSuggestions()).executes((ctx) -> {
   ...
   private static SuggestionProvider playerSuggestions() {
      return (ctx, builder) -> {
         ...
         if (mc.f_91073_ == null) {
            ...
            while(var3.hasNext()) {
               ...
               builder.suggest(p.m_7755_().getString());
            }
   ...
   private static void startTeleport(String playerName) {
      Minecraft mc = Minecraft.m_91087_();
      if (mc.f_91074_ != null && mc.f_91073_ != null) {
         if (active) {
            sendMessage("已经有正在传送运行中 可以先使用 /lexis client tpgoto stop 就停止！");
         } else {
            Player target = null;
            Iterator var3 = mc.f_91073_.m_6907_().iterator();
            ...
            while(var3.hasNext()) {
               Player p = (Player)var3.next();
               if (p.m_7755_().getString().equalsIgnoreCase(playerName)) {
                  target = p;
                  break;
               }
            }
            ...
            if (target == null) {
               sendMessage("找不到玩家 " + playerName);
            } else {
               targetPlayer = target;
               targetStartPos = target.m_20183_();
               active = true;
               sendMessage("正在计算前往 " + targetPlayer.m_7755_().getString() + " 的路径...");
               CompletableFuture.supplyAsync(() -> {
                  return computePathToTarget(mc, targetStartPos);
               }).thenAccept((px) -> {
                  if (px != null && px.size() >= 2) {
                     mc.execute(() -> {
                        path = px;
                        pathIndex = 1;
                        startPathRendering(px);
                        mc.execute(() -> {
                           try {
                              Thread.sleep(1000L);
   ...
   private static List computePathToTarget(Minecraft mc, BlockPos targetBlockPos) {
      if (mc.f_91074_ == null) {
         return null;
      } else {
         Vec3 start = mc.f_91074_.m_20182_();
         Vec3 end = new Vec3((double)targetBlockPos.m_123341_() + 0.5, (double)targetBlockPos.m_123342_() + 0.01, (double)targetBlockPos.m_123343_() + 0.5);
      ...
      if (mc.f_91074_ != null) {
         if (active && moving) {
            if (targetStartPos == null) {
               ...
               Vec3 targetPos = new Vec3((double)targetStartPos.m_123341_() + 0.5, (double)targetStartPos.m_123342_() + 0.01, (double)targetStartPos.m_123343_() + 0.5);
               if (mc.f_91074_.m_20182_().m_82554_(targetPos) <= 1.5) {
                  String var10000 = targetPlayer != null ? targetPlayer.m_7755_().getString() : "目标";
                  stopWalking("已经到了 " + var10000 + " 附近");
               } else {
