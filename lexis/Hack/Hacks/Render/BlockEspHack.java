...
import lexis.Hack.Hackutil.BlockEsp.BlockSelectScreen;
...
public class BlockEspHack extends Hack {
   private EspStyle style;
   private final Map blockSettings;
   private final Map renderPositions;
   private HackConfig config;
   private static final String CONFIG_KEY = "方块透视";
   ...
   public BlockEspHack() {
      super("方块透视", "透视找方块", Hack.Category.RENDER, true);
      this.style = BlockEspHack.EspStyle.LINES_AND_BOXES;
      this.blockSettings = new ConcurrentHashMap();
      ...
      this.addSetting(new Hack.Setting("透视方块", "选择要透视的方块", "选择添加透视的方块", () -> {
         if (mc != null) {
            mc.m_91152_(new BlockSelectScreen(this, this.blockSettings));
         }
      ...
      this.startTaskThread();
   }

   private void loadConfig() {
      String styleStr = this.config.getStringSetting("方块透视", "显示样式", "连线+方框");
   ...
   private void loadBlockSettings() {
      try {
         ...
         if (saveData.containsKey("blocks")) {
            Map blocks = (Map)saveData.get("blocks");

            BlockSettings bs;
            Map.Entry entry;
            for(var4 = blocks.entrySet().iterator(); var4.hasNext(); this.blockSettings.put((String)entry.getKey(), bs)) {
               entry = (Map.Entry)var4.next();
               bs = new BlockSettings();
               bs.enabled = (Boolean)((Map)entry.getValue()).get("enabled");
               bs.color = ((Double)((Map)entry.getValue()).get("color")).intValue();
               if (((Map)entry.getValue()).containsKey("maxDistance")) {
                  bs.maxDistance = (Double)((Map)entry.getValue()).get("maxDistance");
               }
            }
         }
   ...
   public void saveBlockSettings() {
      try {
         ...
         Iterator var3 = this.blockSettings.entrySet().iterator();

         while(var3.hasNext()) {
            Map.Entry entry = (Map.Entry)var3.next();
            Map blockData = new HashMap();
            blockData.put("enabled", ((BlockSettings)entry.getValue()).enabled);
            blockData.put("color", ((BlockSettings)entry.getValue()).color);
            blockData.put("maxDistance", ((BlockSettings)entry.getValue()).maxDistance);
            blocks.put((String)entry.getKey(), blockData);
         }

         saveData.put("blocks", blocks);
   ...
   private EspStyle getStyleFromString(String name) {
      EspStyle[] var2 = BlockEspHack.EspStyle.values();
      ...
      return BlockEspHack.EspStyle.LINES_AND_BOXES;
   }
                              ...
                              for(int z = 0; z < 16; ++z) {
                                 for(int y = minY; y < maxY; ++y) {
                                    ...
                                    if (block != Blocks.f_50016_) {
                                       ...
                                       if (id != null) {
                                          BlockSettings settings = (BlockSettings)this.blockSettings.get(id.toString());
                                          if (settings != null && settings.enabled) {
                                             this.renderPositions.put(pos.m_7949_(), settings);
                                          }
                                       }
                                    }
                                 }
   ...
   public void enableBlockType(String blockId) {
      BlockSettings settings = (BlockSettings)this.blockSettings.get(blockId);
      if (settings != null) {
   ...
   public void onUpdate() {
      if (mc.f_91074_ != null && mc.f_91073_ != null) {
         ...
         this.renderPositions.entrySet().removeIf((entry) -> {
            double distSq = Vec3.m_82512_((Vec3i)entry.getKey()).m_82557_(playerPos);
            return distSq > ((BlockSettings)entry.getValue()).maxDistance * ((BlockSettings)entry.getValue()).maxDistance;
         });
      }
   }
      ...
      if (this.isEnabled() && mc.f_91074_ != null) {
         ...
         while(true) {
            ...
            do {
               ...
               settings = (BlockSettings)entry.getValue();
               BlockState state = mc.f_91073_.m_8055_(pos);
               shape = state.m_60808_(mc.f_91073_, pos);
            } while(shape.m_83281_());

            float[] color = settings.getColorF();
   ...
   public Map getBlockSettings() {
      return this.blockSettings;
   }

   static {
      CONFIG_FILE = new File(CONFIG_DIR, "blockesp.json");
