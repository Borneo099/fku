...
public class DuplicatorManager {
    ...
    private void tick() {
        ...
        switch (this.phase) {
            ...
            case ARMING: {
                ...
                if (slot != 0) {
                    mc.gameMode.handleInventoryMouseClick(player.containerMenu.containerId, 36 + slot, 0, ClickType.SWAP, (Player)player);
                }
                mc.gameMode.useItem((Player)player, InteractionHand.MAIN_HAND);
            ...
            case DUPING: {
                try {
                    mc.gameMode.handleInventoryMouseClick(player.containerMenu.containerId, 3, 0, ClickType.SWAP, (Player)player);
                    if (cfg.dropTridents) {
                        mc.gameMode.handleInventoryMouseClick(player.containerMenu.containerId, 44, 0, ClickType.THROW, (Player)player);
                    }
                    mc.getConnection().send((Packet)new ServerboundPlayerActionPacket(ServerboundPlayerActionPacket.Action.RELEASE_USE_ITEM, BlockPos.ZERO, Direction.DOWN, 0));
                    ...
                    player.displayClientMessage(Component.literal((String)"\u00a7c[\u4e09\u53c9\u621f\u590d\u5236] \u8fde\u7eed\u5931\u8d253\u6b21"), true);
                    this.phase = Phase.IDLE;
                    break;
                }
                this.phase = Phase.COOLDOWN;
    ...
    private int findBestWeaponSlot(LocalPlayer player) {
        ...
        for (int i = 0; i < 9; ++i) {
            ...
            int riptide = EnchantmentHelper.getTagEnchantmentLevel((Enchantment)Enchantments.RIPTIDE, (ItemStack)stack);
            if (riptide > 0) {
                if (hasRiptideWarned) continue;
                hasRiptideWarned = true;
                player.displayClientMessage(Component.literal((String)"\u00a7e[\u4e09\u53c9\u621f\u590d\u5236] \u6fc0\u6d41\u9644\u9b54\u8df3\u8fc7"), true);
                continue;
            }
