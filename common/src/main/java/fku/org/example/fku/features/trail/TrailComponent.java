...
/**
 * 拖尾特效GUI开关组件 — 左键开关，右键打开详细配置界面
 * 该组件由赛博教员实现
 */
public class TrailComponent extends ToggleComponent {
    ...
    public TrailComponent(int x, int y, int width, int height) {
        super(x, y, width, height, "拖尾特效");
    }

    @Override
    protected boolean isEnabled() {
        return TrailConfig.getInstance().enabled;
    }
    ...
    public void render(GuiGraphics g, int mx, int my, float pt) {
        ...
        String displayStr = hotkeyAppend("拖尾特效: " + (enabled ? "开" : "关"));
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        ...
        if (button == 0) {
            ...
            Minecraft.getInstance().setScreen(new TrailConfigScreen());
            return true;
        } else if (button == 2) {
